...
public class Laptop extends Device {
	...
	public void onReceipt(DatagramPacket packet) {
		...
		try {
			...
			/*if not from a phone, its from a laptop which means its for routing table stuff*/
				...
				case "R":
					//if message came back as R and src is us then save to our routing table.
					if(src.equals(String.valueOf(sourcePort))){
						int posInTable = Integer.parseInt(stampsData[stampsData.length-1]);
						routeTable.add(posInTable-1, stamps.substring(2,stamps.length()-1));
						terminal.println(stampsData[stampsData.length-1]+"--"+stamps.substring(2,stamps.length()-1));
					}
					//send back 
					else{
						dstAddress= new InetSocketAddress("localhost", Integer.parseInt(stampsData[step]));
	...
	private void phoneRelay(DatagramPacket packet) {
		try {
			...
			/*find next closest device to destination*/
				...
				String path = routeTable.get(dest-1);
				System.out.println(path);
				if(path.length()==0)
				{
					dstAddress= new InetSocketAddress("localhost", dest);
				}else{
					dstAddress= new InetSocketAddress("localhost", path.charAt(0)-'0');
				}
	...
	public synchronized void start() throws Exception {
		this.wait(waitTime);
		for(int i : portsInRange){
			dstAddress= new InetSocketAddress("localhost", i);
